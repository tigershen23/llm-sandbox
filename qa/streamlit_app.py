...
# Just put all the data in the SAME DIRECTORY DUDE

#region marketing site supporting code
# Set up document QA index
@st.cache(allow_output_mutation=True)
def get_marketing_site_index():
    welcome_marketing_site_data = SimpleDirectoryReader(path +"/data/welcome_blog/pages").load_data()
...
@st.cache(allow_output_mutation=True)
def get_blog_index():
    welcome_blog_data = SimpleDirectoryReader(path + "/data/welcome_marketing/pages").load_data()
...
def get_combined_index():
    combined_data = SimpleDirectoryReader(path + "/data", recursive=True).load_data()
    combined_index = GPTSimpleVectorIndex(combined_data)
