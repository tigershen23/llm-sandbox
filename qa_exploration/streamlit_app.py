...
def get_marketing_site_index():
    welcome_marketing_site_data = SimpleDirectoryReader(
        path +"/data/welcome_blog/pages",
        required_exts=[".jsonl"],
    ).load_data()
...
def get_blog_index():
    welcome_blog_data = SimpleDirectoryReader(
        path + "/data/welcome_marketing/pages",
        required_exts=[".jsonl"],
    ).load_data()
...
def get_zendesk_index():
    welcome_zendesk_data = SimpleDirectoryReader(
        path + "/data/welcome_zendesk/2023-02-06",
        required_exts=[".html"],
    ).load_data()
...
def get_combined_index():
    combined_data = SimpleDirectoryReader(
        path + "/data/website",
        recursive=True,
    ).load_data()
    ...
    with st.expander("Transcript models"):
        ...
        if (marketing_webinar_transcripts_search.button("Go!", key="Marketing Webinar Transcripts Go button") or
            ...
            with st.spinner("Analyzing voices..."):
                marketing_webinar_transcripts_agent = get_transcripts_agent()
